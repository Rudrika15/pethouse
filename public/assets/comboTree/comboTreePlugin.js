...
(function ($, window, document, undefined) {
  ...
  const defaults = {
    ...
    selectAll: false,
    animationTime: 200,
    disabled:0,//this line add by yash
    error:'',//this line add by yash
  };
  ...
  ComboTree.prototype.init = function () {
    ...
    this._wrapper.append(this.options.error);//this line add by yash
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    //   console.log(sourceItem.isSelectable === undefined && this.options.disabled === sourceItem.id)
      //   let isSelectable =sourceItem.isSelectable === undefined ? true : sourceItem.isSelectable;// this line old without change with documentation
      let isSelectable = !(sourceItem.isSelectable === undefined && this.options.disabled === sourceItem.id) ? true : sourceItem.isSelectable; //this line add by yash
  ...
  ComboTree.prototype.refreshInputVal = function () {
    let tmpTitle = "";
    let tmpId = "";//add by yash
    if (this.options.isMultiple) {
      for (let i = 0; i < this._selectedItems.length; i++) {
        tmpTitle += this._selectedItems[i].title;
        tmpId += this._selectedItems[i].id;//add by yash
        if (i < this._selectedItems.length - 1) tmpTitle += ", ";
      }
    } else {
      tmpTitle = this._selectedItem.title;
      tmpId = this._selectedItem.id;//add by yash
    }

    $("#selectedItemId").val(tmpId);//add by yash
